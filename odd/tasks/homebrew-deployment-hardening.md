...
- User chose `split-observation` without lowering final acceptance: current work is renamed OBSERVE-2 and PLAN-3 retains the real change-plan requirement. Writer replaced `plan` command/API with `content-check`/`ContentObservation`, added a real local positive production-reader test and left a three-line superseded `docs/homebrew-deployment-plan.md` notice rather than deleting this newly authored untracked draft. Focused/full uncached Go checks and Darwin CLI cross-compile passed; writer status `partial` for requested doc removal, not a claim of deployment readiness. Independent OBSERVE-2 verification is pending; no publication or device operation.

## Next step

Correct and independently verify OBSERVE-2 as a content-only work unit, then commit locally. PLAN-3 must later produce actual conditional changes and service/data pre-state; do not skip it or proceed to apply/activation. Remote deployment and publication remain separate decisions.
